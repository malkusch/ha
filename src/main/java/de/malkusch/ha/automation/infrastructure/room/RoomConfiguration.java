...
class RoomConfiguration {
    ...
    @Bean
    public OpenWindowRule openWindowRule(RoomProperties properties, ClimateService climateService,
        ...
        return new OpenWindowRule(climateService, rooms, openWindowPropeties.evaluationRate, co2Threshold, buffer);
    }

    @Bean
    public RoomRepository rooms(RoomProperties properties, LightRepository lights) {
        var rooms = new ArrayList<Room>();
        for (var property : properties.rooms) {
            var id = new RoomId(property.id);
            var signalLightId = new LightId(property.signalLight);
            var signalLight = lights.find(signalLightId);
            var room = new Room(id, signalLight);
            rooms.add(room);
        }
